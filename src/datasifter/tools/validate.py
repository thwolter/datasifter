...
def _normalise_date(value: Any) -> tuple[date | None, list[ValidationIssue]]:
    ...
    return None, [
        ValidationIssue(code="type", message=f"Could not parse date value: {text!r}")
    ]
...
def _normalise_int(value: Any) -> tuple[int | None, list[ValidationIssue]]:
    ...
    if cleaned.isdigit() or (cleaned.startswith("-") and cleaned[1:].isdigit()):
        return int(cleaned), []
    return None, [
        ValidationIssue(code="type", message=f"Invalid integer literal: {text!r}")
    ]
...
def _normalise_float(value: Any) -> tuple[float | None, list[ValidationIssue]]:
    ...
    try:
        ...
        return None, [
            ValidationIssue(code="type", message=f"Invalid float literal: {text!r}")
        ]
...
def _validate_enum(value: Any, enum_values: Sequence[str]) -> list[ValidationIssue]:
    ...
    if isinstance(value, str):
        ...
        return [
            ValidationIssue(
                code="enum", message=f"Value {value!r} not in {enum_values!r}"
            )
        ]
...
def _validate_pattern(value: Any, pattern: str) -> list[ValidationIssue]:
    ...
    return [
        ValidationIssue(
            code="pattern", message=f"Value {text!r} does not match pattern {pattern!r}"
        )
    ]


def _validate_range(
    value: Any, constraints: AttributeConstraints
    ...
    if constraints.min_value is not None and numeric_value < constraints.min_value:
        issues.append(
            ValidationIssue(
                code="range",
                message=f"Value {numeric_value} below minimum {constraints.min_value}",
            )
        )
    if constraints.max_value is not None and numeric_value > constraints.max_value:
        issues.append(
            ValidationIssue(
                code="range",
                message=f"Value {numeric_value} above maximum {constraints.max_value}",
            )
        )
    return issues
    ...
    if attr_type == AttributeType.DATE:
        ...
        if isinstance(result, bool):
            ...
            if lowered in {"true", "yes", "1"}:
                ...
                issues.append(
                    ValidationIssue(code="type", message="Invalid boolean literal")
                )
        elif result is None:
            result = None
        else:
        ...
        if result is None:
            ...
            if text is None:
                issues.append(
                    ValidationIssue(code="type", message="List value must be iterable")
                )
                result = []
            else:
        ...
        if result is None:
            ...
            try:
                ...
                issues.append(
                    ValidationIssue(code="type", message="Invalid JSON value")
                )
    else:
        ...
        if result is not None and not isinstance(result, str):
            ...
            if text is None:
                issues.append(
                    ValidationIssue(code="type", message="Value must be string")
                )
                result = None
            else:
                result = text.strip()
