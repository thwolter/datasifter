...
class AttributeConstraints(BaseModel):
    ...
    enum_values: Sequence[str] | None = Field(
        default=None, description="Allowed values for enum types."
    )
    pattern: str | None = Field(
        default=None, description="Regex pattern the value must match."
    )
    min_value: float | None = Field(default=None)
    max_value: float | None = Field(default=None)
    normaliser: str | None = Field(
        default=None, description="Name of the normaliser function to apply."
    )
...
class RetrievalConfig(BaseModel):
    ...
    max_chunks: int = Field(
        default=12, ge=1, description="Max number of chunks to retrieve per attribute."
    )
...
class ExtractionRequest(BaseModel):
    ...
    digest: str | None = Field(
        default=None, description="Digest of the document to extract from."
    )
    collection_name: str | None = Field(
        default=None, description="Vector store collection name for retrieval."
    )
    attributes: Sequence[str] | None = Field(
        default=None, description="Optional subset of attributes to extract."
    )
    model: str | None = Field(
        default=None, description="Override the default extraction model."
    )
